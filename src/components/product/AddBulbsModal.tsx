...
interface AddBulbProps {
    toggleModal: () => void,
    setBulbs: React.Dispatch<React.SetStateAction<ProductBulb[]>>
}

function AddBulbModal({toggleModal, setBulbs}: AddBulbProps) {

    const {axiosInstance} = useAxios();
    const [bulb, setNewBulb] = useState<ProductBulb>({
        id: undefined,
        name: undefined,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        axiosInstance
            .post(process.env.REACT_APP_API_PREFIX + '/products/bulbs/add', bulb)
            .then((response) => {
                setBulbs(prevState => [...prevState, response.data as ProductBulb])
                toggleModal()
            }).catch((error) => {
                                    ...
                                    <div className="grid gap-4 lg:gap-6">
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                                            <div>
                                                ...
                                                <input placeholder="Enter Bulb name"
                                                       name="name"
                                            ...
                                            <div>
                                                ...
                                                <input placeholder="Enter Bulb description"
                                                       name="description"
                                                       onChange={handleChange}
                                                       type="text"
                                                       className="customInput py-2 px-3  w-full border-gray-200 rounded text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400"/>

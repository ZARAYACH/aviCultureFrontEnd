...
interface AddGasCylinderProps {
    toggleModal: () => void,
    setGasCylinder: React.Dispatch<React.SetStateAction<ProductGasCylinder[]>>
}

const AddGasCylinderModal = ({toggleModal, setGasCylinder}: AddGasCylinderProps) => {

    const {axiosInstance} = useAxios();
    const [GasCylinder, setNewGasCylinder] = useState<ProductGasCylinder>({
        id: undefined,
        name: undefined,
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        setNewGasCylinder((prevGasCylinder) => ({
            ...prevGasCylinder,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        axiosInstance
            .post(process.env.REACT_APP_API_PREFIX + '/products/gas-cylinders/add', GasCylinder)
            .then((response) => {
                setGasCylinder(prevState => [...prevState, response.data as ProductGasCylinder])
                toggleModal()
            }).catch((error) => {
            console.error('Failed to add GasCylinder', error);
        });
    };
                                    ...
                                    <div className="grid gap-4 lg:gap-6">
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                                            <div>
                                                ...
                                                <input placeholder="Enter GasCylinde name"
                                                       name="name"
                                            ...
                                            <div>
                                                ...
                                                <input placeholder="Enter GasCylinde description"
                                                       name="description"
                                                       onChange={handleChange}
                                                       type="text"
                                                       className="customInput py-2 px-3  w-full border-gray-200 rounded text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400"/>
...
export default AddGasCylinderModal;
...
const ProductTools = () => {
    ...
    const [productsTools, setProductTools] = useState<ProductTool[]>([]);
    const {axiosInstance} = useAxios();
    const [isAddToolModalOpen, setIsAddToolModalOpen] = useState(false);
    const toggleAddToolModal = () => {
        setIsAddToolModalOpen(!isAddToolModalOpen);
    };

    useEffect(() => {
        axiosInstance
            .get(process.env.REACT_APP_API_PREFIX + "/products/tools")
            .then((response) => {
                setProductTools(response.data);
    ...
    const deleteproductTool = (id: string) => {
        axiosInstance
            .delete(
                process.env.REACT_APP_API_PREFIX + "/products/tools/" + id + "/delete"
            )
            .then((response) => {
                setProductTools(productsTools.filter((productStrawBales, index) => productStrawBales.id !== id));
            })
            .catch((error) => {
                console.error("Error fetching data :", error);
            });
    ...
    return (
        <Fragment>
            {(isAddToolModalOpen &&
                <AddToolModal toggleModal={toggleAddToolModal} setTools={setProductTools}/>)}
                    ...
                    <div className="-m-1.5 overflow-x-auto">
                                ...
                                <div
                                    ...
                                    <div>
                                        <div className="inline-flex gap-x-2">
                                            ...
                                            <button onClick={toggleAddToolModal}
                        ...
                        <span
                          ...
                          ...
                                    ...
                                    {productsTools.map((productTool) => (
                                        <tr key={productTool.id}>
                                            <td className="h-px w-px whitespace-nowrap">
                                                <div className="pl-6 py-3">
                                                    <label htmlFor={productTool.id} className="flex">
                                                        <input
                                                            id={productTool.id}
                                                            type="checkbox"
                                                            className="shrink-0 border-gray-200 rounded text-blue-600 pointer-events-none focus:ring-blue-500 dark:bg-gray-800 dark:border-gray-700 dark:checked:bg-blue-500 dark:checked:border-blue-500 dark:focus:ring-offset-gray-800"
                                            ...
                                            <td className="h-px w-px whitespace-nowrap">
                                                <div className="px-6 py-3">
                                                    <div className="flex items-center gap-x-2">
                                                        <div className="grow">
                              <span className="text-sm text-gray-600 dark:text-gray-400">
                                {productTool.storageBuilding?.name}
                              </span>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <td className="h-px w-px whitespace-nowrap">
                                                <div className="px-6 py-3">
                                                    <div className="flex items-center gap-x-2">
                                                        <div className="grow">
                              <span className="text-sm text-gray-600 dark:text-gray-400">
                                {productTool?.toolCategorie?.name}
                              </span>
                                                        </div>
                                                    </div>
                                                </div>
                                                    ...
                                                    <div
                                                        ...
                                                        >
                                                            ...
                                                            <div className="py-2 first:pt-0 last:pb-0">
                                                                <a
                                                                    onClick={() =>
                                                                        productTool.id && deleteproductTool(productTool.id)
                                                                    }

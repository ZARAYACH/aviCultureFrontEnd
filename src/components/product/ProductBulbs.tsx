...
import ProductBulb from "./modal/ProductBulb";
import AddBulbModal from "./AddBulbsModal";

const ProductBulbs = () => {
    const [isAddBulbModalOpen, setIsAddMedicineModalOpen] = useState(false);
    const toggleAddBulbModal = () => {
        setIsAddMedicineModalOpen(!isAddBulbModalOpen);
    };
    const [productBulbs, setProductBulbs] = useState<ProductBulb[]>([]);
    const {axiosInstance} = useAxios();

    useEffect(() => {
        axiosInstance
            .get(process.env.REACT_APP_API_PREFIX + "/products/bulbs")
            .then((response) => {
                setProductBulbs(response.data);
            })
            .catch((error) => {
    ...
    const deleteProductBulb = (id: string) => {
        axiosInstance
            .delete(
                process.env.REACT_APP_API_PREFIX + "/products/bulbs/" + id + "/delete"
            )
            .then((response) => {
                setProductBulbs(productBulbs.filter((productBulbs, index) => productBulbs.id !== id));
            })
            .catch((error) => {
    ...
    return (
        <Fragment>
            {(isAddBulbModalOpen &&
                <AddBulbModal toggleModal={toggleAddBulbModal} setBulbs={setProductBulbs}/>)}
                            ...
                            <div
                                ...
                                <div>
                                    <div className="inline-flex gap-x-2">
                                        ...
                                        <button onClick={toggleAddBulbModal}
